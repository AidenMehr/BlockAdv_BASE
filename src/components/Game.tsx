...
const Mob = ({ position, type }: { position: [number, number, number], type: 'zombie' | 'skeleton' | 'creeper' }) => {
  ...
  const color = type === 'zombie' ? '#4CAF50' : 
                type === 'skeleton' ? '#BDBDBD' : 
                '#50C878'; // Creeper color
...
const Block = ({ 
  ...
  const handleClick = (event: ThreeEvent<MouseEvent>) => {
    ...
    if (event.button === 2) { // Right click - Place block
      ...
      if (selectedItem?.type === 'block' && selectedItem.quantity > 0 && onPlace) {
    ...
    } else if (event.button === 0 && onMine) { // Left click - Mine
      setHealth(prev => {
...
const GameWorld = () => {
  ...
  const [blocks, setBlocks] = useState<Array<{ position: [number, number, number], color: string }>>([]);
  const [mobs, setMobs] = useState<Array<{ position: [number, number, number], type: 'zombie' | 'skeleton' | 'creeper' }>>([]);
  const { inventory, selectedSlot, removeFromInventory } = useGameStore();

  useEffect(() => {
    // Generate world with proper typing
    const newBlocks: Array<{ position: [number, number, number], color: string }> = [];
    for (let x = -30; x < 30; x += 2) {
      for (let z = -30; z < 30; z += 2) {
        const height = Math.floor(Math.sin(x/10) * Math.cos(z/10) * 3) + 
    ...
    const newMobs: Array<{ position: [number, number, number], type: 'zombie' | 'skeleton' | 'creeper' }> = [];
    for (let i = 0; i < 15; i++) {
      newMobs.push({
        position: [
          Math.random() * 40 - 20,
        ...
        type: Math.random() < 0.33 ? 'zombie' : 
              Math.random() < 0.66 ? 'skeleton' : 'creeper'
      });
    }
    setMobs(newMobs);
  }, []);
  ...
  useFrame((state) => {
    ...
    state.camera.lookAt(playerPos.current[0], playerPos.current[1], playerPos.current[2]);
  });
    ...
    const handleKeyboard = (e: KeyboardEvent) => {
      ...
      switch(e.key.toLowerCase()) {
        ...
        case 'arrowright':
          ...
          break;
        case ' ':
          if (newPos[1] === 0) { // Only jump if on ground
            newPos[1] = 2;
            setTimeout(() => {
  ...
  const handleBlockPlace = (position: [number, number, number], color: string) => {
    const selectedItem = inventory[selectedSlot];
    if (selectedItem?.type === 'block' && selectedItem.quantity > 0) {
      setBlocks(prev => [...prev, { position, color: selectedItem.color || color }]);
